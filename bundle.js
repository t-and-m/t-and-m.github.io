...
var Firebase, database, debounce, jQuery;
...
jQuery((function(_this) {
  return function($) {
    var enter, line, menu, moveTo, reset;
    menu = $('#menu');
    ...
    reset = debounce((function() {
      return moveTo(menu.find('.caption'));
    }), 200);
    enter = function(event) {
      reset.cancel();
      return moveTo(event.target);
    };
    menu.find('li').hover(enter, reset);
    return $(window).on('load', reset).on('resize', reset);
  };
})(this));
  ...
  return function($) {
    ...
    return form.on('submit', function(event) {
      ...
      form.find('input, textarea').each(function(i, element) {
        return inquiry[element.name] = element.value;
      });
      return database.push(inquiry, function(error) {
        if (error) {
          return alert(error);
        }
        form.addClass('sent');
        return thnx.addClass('visible');
      });
    });
  };
})(this));
...
},{"firebase":1,"jquery":2,"lodash":3}]},{},[4])
